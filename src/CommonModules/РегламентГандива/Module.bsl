Функция ОтправитьЗапросГандива(Адрес, Метод, Тело = Неопределено, Авторизация = Ложь, ИспользоватьJSON = Ложь) Экспорт
	...
	Иначе
		...
		Заголовки.Вставить("Accept", "application/json");
	КонецЕсли;	
	Соединение = Новый HTTPСоединение("api-gandiva.tonusnn.ru",443,,,,5,Новый ЗащищенноеСоединениеOpenSSL(Неопределено, Неопределено));
...
Процедура СоздатьИлиИзменитьЗаявкиГандива(МассивЗаявок)
	...
	Для каждого стр из МассивЗаявок Цикл
		...
		Если ДатаПроверка > Константы.ДатаПоследнейЗагрузки.Получить() Тогда
			...
			Документ = НайтиИлиСоздатьДокументЗаявка(стр.Id, стр.Description, ПреобразоватьДату(стр.CreateDate), СоздатьИлиНайтиПользователя(стр.Contractor));
			...
			Для каждого стр из Хэштеги цикл
				...
				Если Не ЗначениеЗаполнено(СтрокаДопРеквизитыМножественные.Значение)	Тогда
					
					НСпр = Справочники.узМеткиЗадач.СоздатьЭлемент();
					НСпр.Наименование = стр;
					НСпр.Записать();
					
					СтрокаДопРеквизитыМножественные.Значение = НСпр.Ссылка;
...
Функция НайтиИлиСоздатьДокументЗаявка(ид, ТекстЗадачи, ДатаЗадачи, Исполнитель)
	
	Текст = " задача №"+ид+" от "+ДатаЗадачи+Символы.ПС+"Текст задачи:"+Символы.ПС+ТекстЗадачи;
	
	Док = Справочники.узЗадачи.НайтиПоКоду(ид);
	Если ЗначениеЗаполнено(Док) Тогда
		Текст = "Изменена "+Текст;
		ОтправитьСообщениеТелеграм(Текст, Исполнитель);
...
Процедура ОтправитьСообщениеТелеграм(Текст, Пользователь)
	
	Если Пользователь.УведомлениеТелеграм И ЗначениеЗаполнено(ПОльзователь.ИДТелеграм) И ЗначениеЗаполнено(Константы.ТокенТелеграм.Получить()) Тогда
		
		Токен = Константы.ТокенТелеграм.Получить();
		ИДПользователя = Пользователь.ИДТелеграм;
		
		Ресурс = "bot" + Токен + "/sendMessage?chat_id=" + СтрЗаменить(ИДПользователя, Символы.НПП, "") +"&text=" + Текст;
